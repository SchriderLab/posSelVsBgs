...
L=int(L)
minTimeSinceSweepMut, maxTimeSinceSweepMut = math.floor(float(minTimeSinceSweepMut)*popSizeRescaleFactor), math.ceil(float(maxTimeSinceSweepMut)*popSizeRescaleFactor)
...
initialPhaseEnd = maxTimeSinceSweepMut+1
sys.stderr.write("running up until maxTimeSinceSweepMut+1 generations ago (%d gen forward, up until %d gen ago)\n" %(len(nlist)-initialPhaseEnd, maxTimeSinceSweepMut))
...
sweepPos = miscFwdpyFuncs.findSweepPosNearTarget(L/2, pop)
acceptRep=False
numAttempts = 0
while not acceptRep:
    timeSinceSweepMut = np.random.randint(minTimeSinceSweepMut, maxTimeSinceSweepMut+1)
    sys.stderr.write("drawing timeSinceSweepMut from [%s, %s]\n" %(minTimeSinceSweepMut, maxTimeSinceSweepMut))
    assert lossCheckTime < timeSinceSweepMut
    sys.stderr.write("sweep mutation time: %d generations ago; selection coefficient: %f; sweep mut position: %g\n" %(timeSinceSweepMut, sweepSelCoeff, sweepPos))
    sys.stderr.write("simulating up to our sweep mut time (evolving from %d to %d)\n" %(len(nlist)-initialPhaseEnd, len(nlist)-timeSinceSweepMut))
    params=mparams.SlocusParams(nregions=nregions, sregions=sregions, recregions=recregions, demography=np.array(nlist[-initialPhaseEnd:-timeSinceSweepMut], dtype=np.uint32), rates=(u*L, u*totSelRegionSize, totalRecRateInMorgans))
    fwdpy11wf.evolve(rng, pop, params)
    miscFwdpyFuncs.addMutAtPosition(pop, rng, sweepPos, sweepSelCoeff, 0.5)
    acceptProb = miscFwdpyFuncs.calcSweepMutAcceptanceProb(timeSinceSweepMut, minTimeSinceSweepMut, maxTimeSinceSweepMut, nlist)
    ...
    sys.stderr.write("simulating for %d gen then checking to see if it was lost (evolving from %d to %d)\n" %(lossCheckTime, len(nlist)-timeSinceSweepMut, len(nlist)-(timeSinceSweepMut-lossCheckTime)))
    params=mparams.SlocusParams(nregions=nregions, sregions=sregions, recregions=recregions, demography=np.array(nlist[-timeSinceSweepMut:-(timeSinceSweepMut-lossCheckTime)], dtype=np.uint32), rates=(u*L, u*totSelRegionSize, totalRecRateInMorgans))
    fwdpy11wf.evolve(rng, pop, params)
    ...
    if miscFwdpyFuncs.sweepMutLost(pop, sweepPos):
        ...
        sys.stderr.write("sweep mut still segregating; simulating for the remaining %d gen (evolving from %d to %d)\n" %(timeSinceSweepMut-lossCheckTime, len(nlist)-(timeSinceSweepMut-lossCheckTime), len(nlist)))
        params=mparams.SlocusParams(nregions=nregions, sregions=sregions, recregions=recregions, demography=np.array(nlist[-(timeSinceSweepMut-lossCheckTime):], dtype=np.uint32), rates=(u*L, u*totSelRegionSize, totalRecRateInMorgans))
        fwdpy11wf.evolve(rng, pop, params)
...
sys.stderr.write("Final sweep characteristics (re-scaled back to orig): (sweepPos\\tsweepSelCoeff\\ttimeSinceSweepMut\\ttimeSinceFixation)\n")
sys.stderr.write("%s\t%s\t%s\t%s\n" %(sweepPos, sweepSelCoeff*popSizeRescaleFactor, timeSinceSweepMut/popSizeRescaleFactor, (len(nlist)-fixationTime)/popSizeRescaleFactor))
